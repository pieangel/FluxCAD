...
namespace FluxCAD.BricsCAD.Plugin26
{
    public sealed class SheetAnalysisDebugCommands
    {
        ...
        private sealed class CopiedEntityMap
        {
            public int ViewIslandId { get; init; }

            public string SourceHandle { get; init; } = "";
            public string CopiedHandle { get; init; } = "";

            public SheetEntityKind SourceKind { get; init; }
            public SheetEntityRole SourceRole { get; init; }

            public string SourceLayer { get; init; } = "";
            public string CopiedLayer { get; init; } = "";

            public bool IsHiddenOrCenter { get; init; }

            public Bounds2D SourceBounds { get; init; }
            public Bounds2D CopiedBounds { get; init; }
        }
        ...
        public void FluxCopyVisibleOutlineWorkspace()
        {
            ...
            try
            {
                ...
                using (var tr = db.TransactionManager.StartTransaction())
                {
                    ...
                    string sourceLayer = "FLUX_VIEW_COPY_OUT";
                    string targetLayerBase = "FLUX_VISIBLE_OUTLINE";

                    var maps = new List<CopiedEntityMap>();
                    ...
                    foreach (ObjectId id in ms)
                    {
                        ...
                        if (!ent.Layer.StartsWith(sourceLayer))
                            continue;

                        if (IsHiddenOrCenterCadEntity(ent))
                            continue;

                        if (IsTextOrAnnotationCadEntity(ent))
                            continue;
                        ...
                        string targetLayer = EnsureIslandLayer(
                            db, tr, targetLayerBase, islandId);
                        ...
                        maps.Add(new CopiedEntityMap
                        {
                            ViewIslandId = islandId,
                            SourceHandle = ent.Handle.ToString(),
                            CopiedHandle = clone.Handle.ToString(),
                            SourceLayer = ent.Layer,
                            CopiedLayer = clone.Layer,
                            SourceBounds = bounds,
                            CopiedBounds = bounds,
                            IsHiddenOrCenter = false
                        });
                    }

                    tr.Commit();

                    ed.WriteMessage($"\n[FluxCAD] Visible workspace created. Count={maps.Count}");
                }
            }
        ...
        private static string EnsureIslandLayer(
    Database db,
    Transaction tr,
    string baseName,
    int islandId)
        {
            string name = $"{baseName}_I{islandId:000}";
        ...
        private static int ExtractIslandIdFromLayer(string layer)
        {
            if (string.IsNullOrWhiteSpace(layer))
                return -1;

            var idx = layer.LastIndexOf("_I");
            if (idx < 0)
                return -1;

            var num = layer.Substring(idx + 2);
        ...
        private static string EnsureVisibleOutlineWorkspaceLayer(Database db, Transaction tr)
        {
            const string layerName = "FLUX_VISIBLE_OUTLINE_WORK";
            ...
            if (!lt.Has(layerName))
            {
                ...
                {
                    Name = layerName,
                    Color = TeighaColor.FromColorIndex(Teigha.Colors.ColorMethod.ByAci, 2)
                };
        ...
        private static bool IsTextOrAnnotationCadEntity(Entity ent)
        {
        ...
        private static bool IsHiddenOrCenterCadEntity_new(Entity ent)
        {
            if (ent == null)
                return false;

            string layer = NormalizeCadText(ent.Layer);
            string linetype = NormalizeCadText(ent.Linetype);

            if (layer.Contains("CENTER") ||
                layer.Contains("CENTRE") ||
                layer.Contains("CNTR") ||
                layer.Contains("CTR") ||
                layer.Contains("중심") ||
                layer.Contains("CENTERLINE"))
                return true;

            if (layer.Contains("HIDDEN") ||
                layer.Contains("HID") ||
                layer.Contains("숨은") ||
                layer.Contains("은선"))
                return true;

            if (linetype.Contains("CENTER") ||
                linetype.Contains("CENTRE") ||
                linetype.Contains("CNTR") ||
                linetype.Contains("CTR"))
                return true;

            if (linetype.Contains("HIDDEN") ||
                linetype.Contains("HID") ||
                linetype.Contains("DASH") ||
                linetype.Contains("DOT") ||
                linetype.Contains("PHANTOM"))
                return true;
        ...
        private static void DrawOuterSideBandsDebug(
            Database db,
            Transaction tr,
            BlockTableRecord ms,
            string layerName,
            Bounds2D bounds)
        {
            ...
            DrawDebugBoundsBox(db, tr, ms, layerName,
        ...
        private static void DrawDebugBoundsBox(
            Database db,
            Transaction tr,
            BlockTableRecord ms,
            string layerName,
            ...
            short colorIndex = 2)
        {
            ...
            var p1 = new Point3d(bounds.MinX, bounds.MinY, 0);
            var p2 = new Point3d(bounds.MaxX, bounds.MinY, 0);
            var p3 = new Point3d(bounds.MaxX, bounds.MaxY, 0);
            var p4 = new Point3d(bounds.MinX, bounds.MaxY, 0);
        ...
        new Line(p4, p1)
            ...
            foreach (var line in lines)
            {
                ...
                line.Color = Teigha.Colors.Color.FromColorIndex(
    ...
    IReadOnlyList<SheetEntity> semanticEntities)
        ...
        private static bool IsHiddenOrCenterCadEntity_old2(Entity ent)
        {
            if (ent == null)
                return false;
